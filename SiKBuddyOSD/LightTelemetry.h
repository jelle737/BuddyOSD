/*
    LightTelemetry.h - Library for LTM
    Created by Jelle L., September 15, 2017
    Released under GPL3 lisence
*/
...
class LightTelemetry
{
    ...
    public:
        ...
        void init(AltSoftSerial* serial);
        bool read();
        void check();
        void transmit(HardwareSerial* serial);
    ...
    private:
        ...
        HardwareSerial* ltmHardwareSerial;
        AltSoftSerial* ltmSoftwareSerial;

        uint8_t LTMserialBuffer[LIGHTTELEMETRY_GFRAMELENGTH];
        uint8_t LTMreceiverIndex;
        uint8_t LTMcmd;
        uint8_t LTMrcvChecksum;
        uint8_t LTMreadIndex;
        uint8_t LTMwriteIndex;
